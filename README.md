...
Below is a breakdown of every major file and directory in this repository and the specific function it serves.
...
* **Description:** Processes the cleaned data to generate all empirical findings for the live dashboard. It performs the following sequential tasks:
...
### `Paper_static_analysis/` (Directory)
* **Function:** Academic Reproducibility & Preliminary Analysis.
* **Description:** This folder isolates the point-in-time code and data used to generate the specific tables, figures, and preliminary findings presented in the academic paper. It is separated from the live automated pipeline to ensure production stability. It contains three subdirectories:
    * **`Data_Preliminary/`:** Contains the scripts and generated figures for exploratory data analysis, including summary statistics, signal concentration charts, missingness heatmaps, and correlation matrices.
    * **`Methodology_Part1/`:** Contains the Jupyter notebooks (`ols+dlmrevised.ipynb`, `sharpe+event+backtestrevised.ipynb`) used to execute the baseline linear regressions, distributed lag models, event studies, and initial backtests.
    * **`Methodology_Part2/`:** Contains the Python scripts (`ML_analysis.py`) and static outputs used for the non-linear machine learning analysis, including the LightGBM models, impulse response functions, and feature importance rankings.
