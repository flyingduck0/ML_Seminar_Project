...
# 2. Identify which columns contain 'inf_yearly' (ignoring uppercase/lowercase)
cols_to_drop = [col for col in df.columns if 'inf_yearly' in col.lower()]

# 3. Drop those columns and create the final dataset
Final_Pipeline_Data = df.drop(columns=cols_to_drop)

# 4. Save the final output to a new CSV
Final_Pipeline_Data.to_csv('Final_Pipeline_Data.csv', index=False)

print("Columns dropped. Final_Pipeline_Data.csv has been saved.")
# %%